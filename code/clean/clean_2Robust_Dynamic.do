...
use hosp_weathter_full_cdg0s, clear
	...
	*- Generate variables of forecast erros for parametric estimation
			...
			*translate to average cummulative effect
			...
			replace  L`j'_rT_CB`bins'= r_TempHigh_L`j' 
			replace  L`j'_elow_CB`bins'= e_low_L`j'
			replace  L`j'_ehigh_CB`bins'= e_high_L`j'
			replace  L`j'_ishigh_CB`bins'= is_high_L`j'
	...
	*- Sample processing: Remove city-year data with missing data
	...
	save treated_hosp_weathter_full_cdg0_Dynamic, replace 
