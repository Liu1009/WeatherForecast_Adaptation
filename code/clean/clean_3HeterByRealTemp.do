...
use hosp_weathter_full_cdg0s, clear
	...
	*- Generate variables of forecast erros for parametric estimation
			...
			forval j=0(1)`periods' {
				...
				tab bin_r_TempHigh_L`j', gen(r_TempHigh_L`j'_B) //1-9
					...
					forval bins =1(1)9 {
						cap drop  e_low_L`j'_B`bins'
						gen e_low_L`j'_B`bins'= e_low_L`j'*r_TempHigh_L`j'_B`bins'
						gen e_high_L`j'_B`bins'= e_high_L`j'*r_TempHigh_L`j'_B`bins'
						gen is_high_L`j'_B`bins'= is_high_L`j'*r_TempHigh_L`j'_B`bins'
					}
			...
			*translate to average cummulative effect
			...
			replace  L0_rT_CB`bins'=r_TempHigh_L0_B`bins'
			replace  L0_elow_CB`bins'=e_low_L0_B`bins' 
			replace  L0_ehigh_CB`bins'=e_high_L0_B`bins' 
			replace  L0_ishigh_CB`bins'=is_high_L0_B`bins'
			
			forval j=1(1)`periods' {
				
			replace  L`j'_rT_CB`bins'= r_TempHigh_L`j'_B`bins' -L0_rT_CB`bins'
			replace  L`j'_elow_CB`bins'= e_low_L`j'_B`bins'-L0_elow_CB`bins'
			replace  L`j'_ehigh_CB`bins'= e_high_L`j'_B`bins'-L0_ehigh_CB`bins'
			replace  L`j'_ishigh_CB`bins'= is_high_L`j'_B`bins'-L0_ishigh_CB`bins'
			
			}
			
			replace  L0_elow_CB`bins'=e_low_L0_B`bins' *`periods'
			replace  L0_ehigh_CB`bins'=e_high_L0_B`bins' *`periods'
			replace  L0_ishigh_CB`bins'=is_high_L0_B`bins'*`periods'
	...
	*- Sample processing: Remove city-year data with missing data
	...
	save treated_hosp_weathter_full_cdg0_HeterByRealized, replace 
