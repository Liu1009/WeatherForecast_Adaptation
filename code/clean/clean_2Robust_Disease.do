...
use hosp_weathter_full_cdgs_DiseaseTypes, clear
			...
			program define bin_group
				...
				*- for Bin_i=0
				...
				forval j=1(1)`periods' {
					...
					bys city_id groups type_dises (date_ymd): replace L`j'_`varname'_B0=1 if `varname'[_n-`j'] < `first_interval'
			    ...
			    foreach low in `intervals' {
			        ...
			        if "`high'" != "" {
						...
						forval j=0(1)`periods' {
							...
							bys city_id groups type_dises (date_ymd): replace L`j'_`varname'_B`i'=1 if  `varname'[_n-`j'] >= `low' & `varname'[_n-`j'] < `high'
						...
						bys city_id groups type_dises (date_ymd): replace L`j'_`varname'_B`i'=1 if `varname'[_n-`j'] >= `low' & !mi(`varname'[_n-`j'])
	...
	*- Generate bins of forecast errors for non-parametric estimation
			...
			forval j=0(1)`periods' {
				...
				gen r_TempHigh_L`j'=. 
				bys city_id groups type_dises (date_ymd): replace  r_TempHigh_L`j'= r_TempHigh[_n-`j']
	...
	*- Generate weather & pollution bins
			...
			foreach var in r_WindAvg r_PM25 r_RainVol r_HumiAvg { //r_PresAvg 
				...
				forval j=0(1)`periods' {
					...
					cap drop `var'_L`j'
					gen `var'_L`j'=. 
					bys city_id groups type_dises (date_ymd): replace  `var'_L`j'= `var'[_n-`j']
	...
	*- Generate variables of forecast erros for parametric estimation
			...
			forval j=0(1)`periods' {
				...
				bys city_id groups type_dises (date_ymd): gen dif_TempHigh_L`j'=dif_TempHigh[_n-`j']
	...
	*- Sample processing: Remove city-year data with missing data
	...
	save treated_hosp_weathter_full_cdg_Disease.dta, replace 
